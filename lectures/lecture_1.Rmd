...
DiagrammeR::grViz("
  digraph {
    ...
    gender -> math_results [label= β1]
  }", height = 150, width = 200)
```
]

Hypothesis (one-tailed): 
- $H_a$: avg(female) > avg(male)

*if female is first (i.e. coded 1) and male is second (i.e. coded 2), we expect the line joining avg(female) and avg(male) to decrease, therefore $\beta_1 < 0$*

- $H_0$: avg(female) = avg(male)

*if female is first (i.e. coded 1) and male is second (i.e. coded 2), we expect the line joining avg(female) and avg(male) to be stable, therefore $\beta_1 = 0$*
...
DiagrammeR::grViz("
  digraph {
    ...
    age_group -> math_results [label= β1]
  }", height = 200, width = 200)
```
]
...
DiagrammeR::grViz("
  digraph {
    ...
    node [shape = circle]
    IQ; math_results;'IQ*gender'; gender

    gender -> math_results [label= β2]
    IQ -> math_results [label= β1]
    'IQ*gender' -> math_results [label= β3]
