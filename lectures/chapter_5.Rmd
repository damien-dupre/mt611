...
my_data <- data.frame(power = my_power, cases = my_knowledge)

easy_viz <- barplot(height = my_power, names.arg = my_knowledge)
```
...
`r faa("arrow-circle-right", animate="horizontal", speed="slow", color="blue")` Note: Not all data is suitable for all types of geometries. It is to you to find the one that corresponds to your data
...
- **Rmarkdown:** a tool for mixing R code with markdown 
...
## Everyone: Okaaaay, but this seems really boring?<br>Me: ... hm, yeah, but let me show you where it takes you later!
